    ...
    class STORMKIT_API Buffer {
      ...
      public:
        struct CreateInfo {
            ...
            usize              size;
            MemoryPropertyFlag property = MemoryPropertyFlag::HOST_VISIBLE | MemoryPropertyFlag::HOST_COHERENT;
        };
        ...
        static auto create(const Device& device, const CreateInfo& info, bool persistently_mapped = false) noexcept
          -> Expected<Buffer>;
        static auto allocate(const Device& device, const CreateInfo& info, bool persistently_mapped = false) noexcept
          -> Expected<Heap<Buffer>>;
        ...
        auto data(this Self& self, usize size) noexcept
          -> core::meta::If<core::meta::IsConst<Self>, std::span<byte>, std::span<const byte>>;
        ...
        Buffer(const Device& device, const CreateInfo& info, bool persistently_mapping, PrivateFuncTag) noexcept;
...
namespace stormkit::gpu {
    /////////////////////////////////////
    /////////////////////////////////////
    inline Buffer::Buffer(const Device& device, const CreateInfo& info, bool persistently_mapped, PrivateFuncTag) noexcept
        : m_usages { info.usages },
          m_size { info.size },
          m_memory_property { info.property },
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto Buffer::create(const Device& device, const CreateInfo& info, bool persistently_mapped) noexcept
      -> Expected<Buffer> {
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto Buffer::allocate(const Device& device, const CreateInfo& info, bool persistently_mapped) noexcept
      -> Expected<Heap<Buffer>> {
        auto buffer = std::make_unique<Buffer>(device, info, persistently_mapped, PrivateFuncTag {});
        return buffer->do_init(info.property).transform(core::monadic::consume(buffer));
    }
    ...
    inline auto Buffer::data(this Self& self, usize size) noexcept
      -> core::meta::If<core::meta::IsConst<Self>, std::span<byte>, std::span<const byte>> {
        ...
        using Out = core::meta::If<core::meta::IsConst<Self>, std::span<byte>, std::span<const byte>>;
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto Buffer::upload(std::span<const byte> data, ioffset offset) noexcept -> Expected<void> {
        ...
        return map(offset, stdr::size(data)).transform([this, &data](auto&& out) noexcept -> void {
            stdr::copy(data, stdr::begin(out));
            unmap();
        });
    }
