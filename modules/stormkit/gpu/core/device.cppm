...
export {
    namespace stormkit::gpu {
        ...
        class STORMKIT_GPU_API Device {
          ...
          public:
            ...
            auto reset_fences(std::span<const Ref<const Fence>> fences) const noexcept -> Expected<void>;
    ...
    struct std::formatter<gpu::Device::QueueEntry, CharT> {
        ...
        [[nodiscard]]
        auto format(const gpu::Device::QueueEntry& queue, FormatContext& ctx) const noexcept -> decltype(ctx.out());
    };
}
...
namespace stormkit::gpu {
    ...
    inline auto Device::set_object_name(const T& object, std::string_view name) const -> Expected<void> {
        ...
        return set_object_name(std::bit_cast<u64>(static_cast<const void*>(vk_object)), T::DEBUG_TYPE, name);
    }
...
template<class CharT>
template<class ParseContext>
constexpr auto std::formatter<gpu::Device::QueueEntry, CharT>::parse(ParseContext& ctx) noexcept -> ParseContext::iterator {
    return ctx.begin();
}
...
auto std::formatter<gpu::Device::QueueEntry, CharT>::format(const gpu::Device::QueueEntry& queue,
    ...
    auto&& out = ctx.out();
    return format_to(out, "[Device::QueueEntry: .id = {}, .count = {}, .flags = {}]", queue.id, queue.count, queue.flags);
}
