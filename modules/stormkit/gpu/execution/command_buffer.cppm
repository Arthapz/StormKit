...
export namespace stormkit::gpu {
    ...
    class STORMKIT_API Queue {
      ...
      public:
        ...
        static auto allocate(const Device& device, const Device::QueueEntry& entry) noexcept -> Heap<Queue>;
        ~Queue() noexcept;
        ...
        auto submit(std::span<const SubmitInfo> submit_infos, OptionalRef<const Fence> fence = std::nullopt) const noexcept
          -> Expected<void>;

        auto submit(const SubmitInfo& submit_info, OptionalRef<const Fence> fence = std::nullopt) const noexcept
          -> Expected<void>;
    ...
    class STORMKIT_API CommandBuffer {
      ...
      public:
        ...
        using Deleter = std::function<void(VkDevice, VkCommandPool, const VolkDeviceTable&, VkCommandBuffer)>;
        ...
        auto reset() noexcept -> Expected<Ref<CommandBuffer>>;
        ...
        auto end_rendering() noexcept -> CommandBuffer&;

        auto bind_pipeline(const Pipeline& pipeline) noexcept -> CommandBuffer&;
        auto set_viewport(u32 first_viewport, std::span<const Viewport> viewports) noexcept -> CommandBuffer&;
        auto set_scissor(u32 first_scissor, std::span<const Scissor> scissors) noexcept -> CommandBuffer&;
        auto set_line_width(f32 width) noexcept -> CommandBuffer&;
        auto set_depth_bias(f32 constant_factor, f32 clamp, f32 slope_factor) noexcept -> CommandBuffer&;
        auto set_blend_constants(std::span<const f32> constants) noexcept -> CommandBuffer&;
        ...
        auto dispatch(u32 group_count_x, u32 group_count_y, u32 group_count_z) noexcept -> CommandBuffer&;

        auto draw(u32 vertex_count, u32 instance_count = 1u, u32 first_vertex = 0, u32 first_instance = 0) noexcept
          -> CommandBuffer&;
        ...
        auto draw_indirect(const Buffer& buffer, usize offset, u32 draw_count, u32 stride) noexcept -> CommandBuffer&;
        auto draw_indexed_indirect(const Buffer& buffer, usize offset, u32 draw_count, u32 stride) noexcept -> CommandBuffer&;

        auto bind_vertex_buffers(std::span<const Ref<const Buffer>> buffers, std::span<const u64> offsets) noexcept
          -> CommandBuffer&;
        auto bind_index_buffer(const Buffer& buffer, u64 offset = 0, bool large_indices = false) noexcept -> CommandBuffer&;
        auto bind_descriptor_sets(const Pipeline&                           pipeline,
                                  const PipelineLayout&                     layout,
                                  std::span<const Ref<const DescriptorSet>> descriptor_sets,
                                  std::span<const u32>                      dynamic_offsets = {}) noexcept -> CommandBuffer&;

        auto copy_buffer(const Buffer& src, const Buffer& dst, usize size, u64 src_offset = 0u, u64 dst_offset = 0u) noexcept
          -> CommandBuffer&;
        auto copy_buffer_to_image(const Buffer&                    src,
                                  const Image&                     dst,
                                  std::span<const BufferImageCopy> buffer_image_copies = {}) noexcept -> CommandBuffer&;
        auto copy_image_to_buffer(const Image&                     src,
                                  const Buffer&                    dst,
                                  std::span<const BufferImageCopy> buffer_image_copies = {}) noexcept -> CommandBuffer&;
        ...
        auto resolve_image(const Image&                  src,
                           ...
                           ImageLayout                   dst_layout,
                           const ImageSubresourceLayers& src_subresource_layers = {},
                           const ImageSubresourceLayers& dst_subresource_layers = {}) noexcept -> CommandBuffer&;
        ...
        auto transition_image_layout(const Image&                 image,
                                     ImageLayout                  src_layout,
                                     ImageLayout                  dst_layout,
                                     const ImageSubresourceRange& subresource_range = {}) noexcept -> CommandBuffer&;

        auto execute_sub_command_buffers(std::span<const Ref<const CommandBuffer>> commandbuffers) noexcept -> CommandBuffer&;
        ...
        auto pipeline_barrier(PipelineStageFlag                    src_mask,
                              ...
                              std::span<const ImageMemoryBarrier>  image_memory_barriers) noexcept -> CommandBuffer&;
...
namespace stormkit::gpu {
    ...
    inline Queue::Queue(const Device& device, const Device::QueueEntry& entry, PrivateFuncTag) noexcept
        : m_entry { entry }, m_vk_device { device.native_handle() }, m_vk_device_table { as_ref(device.device_table()) } {
        m_vk_handle = vk_call<VkQueue>(m_vk_device_table->vkGetDeviceQueue, m_vk_device, m_entry.id, 0);
    }
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto Queue::create(const Device& device, const Device::QueueEntry& entry) noexcept -> Queue {
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto Queue::allocate(const Device& device, const Device::QueueEntry& entry) noexcept -> Heap<Queue> {
    ...
    inline auto Queue::wait_idle() const noexcept -> Expected<void> {
        return vk_call(m_vk_device_table->vkQueueWaitIdle, m_vk_handle).transform_error(monadic::from_vk<Result>());
    }
    ...
    //
    STORMKIT_FORCE_INLINE
    inline auto Queue::submit(const SubmitInfo& submit_info, OptionalRef<const Fence> fence) const noexcept -> Expected<void> {
    ...
    inline CommandBuffer::CommandBuffer(VkDevice                   device,
        ...
        : m_level { level },
          m_vk_device { device },
          m_vk_device_table { device_table },
          m_vk_pool { pool },
          m_vk_handle {
              { [vk_device       = m_vk_device,
                 vk_pool         = m_vk_pool,
                 vk_device_table = *m_vk_device_table,
                 deleter = std::move(deleter)](auto handle) noexcept { deleter(vk_device, vk_pool, vk_device_table, handle); } }
          } {
    ...
    inline auto CommandBuffer::create(VkDevice                   device,
        ...
        return CommandBuffer { device, pool, device_table, level, std::move(cmb), std::move(deleter), PrivateFuncTag {} };
    }
    ...
    inline auto CommandBuffer::allocate(VkDevice                   device,
        ...
        return *allocate<CommandBuffer>(device, pool, device_table, level, std::move(cmb), std::move(deleter), PrivateFuncTag {})
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto CommandBuffer::reset() noexcept -> Expected<Ref<CommandBuffer>> {
        return vk_call(m_vk_device_table->vkResetCommandBuffer, m_vk_handle, 0)
          .transform([this, &self = *this] {
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto CommandBuffer::end() noexcept -> Expected<Ref<CommandBuffer>> {
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto CommandBuffer::end_rendering() noexcept -> CommandBuffer& {
        EXPECTS(m_state == State::RECORDING);

        vk_call(m_vk_device_table->vkCmdEndRendering, m_vk_handle);
        return *this;
    }
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto CommandBuffer::set_line_width(f32 width) noexcept -> CommandBuffer& {
        EXPECTS(m_state == State::RECORDING);
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto CommandBuffer::set_depth_bias(f32 constant_factor, f32 clamp, f32 slope_factor) noexcept -> CommandBuffer& {
        EXPECTS(m_state == State::RECORDING);

        vk_call(m_vk_device_table->vkCmdSetDepthBias, m_vk_handle, constant_factor, clamp, slope_factor);
        return *this;
    }
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto CommandBuffer::set_blend_constants(std::span<const f32> constants) noexcept -> CommandBuffer& {
        EXPECTS(m_state == State::RECORDING);
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto CommandBuffer::set_depth_bounds(f32 min, f32 max) noexcept -> CommandBuffer& {
        EXPECTS(m_state == State::RECORDING);
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto CommandBuffer::set_stencil_compare_mask(StencilFaceFlag face, u32 mask) noexcept -> CommandBuffer& {
        EXPECTS(m_state == State::RECORDING);

        vk_call(m_vk_device_table->vkCmdSetStencilCompareMask, m_vk_handle, to_vk<VkStencilFaceFlagBits>(face), mask);
        return *this;
    }
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto CommandBuffer::set_stencil_write_mask(StencilFaceFlag face, u32 mask) noexcept -> CommandBuffer& {
        EXPECTS(m_state == State::RECORDING);

        vk_call(m_vk_device_table->vkCmdSetStencilWriteMask, m_vk_handle, to_vk<VkStencilFaceFlagBits>(face), mask);
        return *this;
    }
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto CommandBuffer::set_stencil_reference(StencilFaceFlag face, u32 reference) noexcept -> CommandBuffer& {
        EXPECTS(m_state == State::RECORDING);

        vk_call(m_vk_device_table->vkCmdSetStencilReference, m_vk_handle, to_vk<VkStencilFaceFlagBits>(face), reference);
        return *this;
    }
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto CommandBuffer::dispatch(u32 group_count_x, u32 group_count_y, u32 group_count_z) noexcept -> CommandBuffer& {
        EXPECTS(m_state == State::RECORDING);

        vk_call(m_vk_device_table->vkCmdDispatch, m_vk_handle, group_count_x, group_count_y, group_count_z);
        return *this;
    }
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto CommandBuffer::draw(u32 vertex_count, u32 instance_count, u32 first_vertex, u32 first_instance) noexcept
      -> CommandBuffer& {
        EXPECTS(m_state == State::RECORDING);
        EXPECTS(vertex_count > 0);

        vk_call(m_vk_device_table->vkCmdDraw, m_vk_handle, vertex_count, instance_count, first_vertex, first_instance);
        return *this;
    }
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto CommandBuffer::draw_indirect(const Buffer& buffer, usize offset, u32 draw_count, u32 stride) noexcept
      -> CommandBuffer& {
        EXPECTS(m_state == State::RECORDING);
        EXPECTS(draw_count > 0);

        vk_call(m_vk_device_table->vkCmdDrawIndirect, m_vk_handle, to_vk(buffer), offset, draw_count, stride);
        return *this;
    }
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto CommandBuffer::draw_indexed_indirect(const Buffer& buffer, usize offset, u32 draw_count, u32 stride) noexcept
      -> CommandBuffer& {
        EXPECTS(m_state == State::RECORDING);
        EXPECTS(draw_count > 0);

        vk_call(m_vk_device_table->vkCmdDrawIndexedIndirect, m_vk_handle, to_vk(buffer), offset, draw_count, stride);
        return *this;
    }
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto CommandBuffer::bind_index_buffer(const Buffer& buffer, u64 offset, bool large_indices) noexcept
      -> CommandBuffer& {
        EXPECTS(m_state == State::RECORDING);
    ...
    /////////////////////////////////////
    STORMKIT_FORCE_INLINE
    inline auto CommandPool::allocate(const Device& device) noexcept -> Expected<Heap<CommandPool>> {
        auto pool = *core::allocate<CommandPool>(device, PrivateFuncTag {}).transform_error(core::monadic::assert());
        return pool->do_init().transform(core::monadic::consume(pool));
    }
    ...
    inline auto CommandPool::create_command_buffer(CommandBufferLevel level) const noexcept -> Expected<CommandBuffer> {
        return create_command_buffers(1, level).transform([](auto&& cmbs) static noexcept { return std::move(cmbs.front()); });
    }
    ...
    inline auto CommandPool::allocate_command_buffer(CommandBufferLevel level) const noexcept -> Expected<Heap<CommandBuffer>> {
        return allocate_command_buffers(1, level).transform([](auto&& cmbs) static noexcept { return std::move(cmbs.front()); });
    }
