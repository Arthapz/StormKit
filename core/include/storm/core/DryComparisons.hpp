// from https://github.com/rollbear/dry-comparisonsS
...
namespace storm::core {
    namespace internal {
        ...
        struct bound {
            ...
            constexpr bound(F_ &&f_, As &&... as)
                : f(std::forward<F_>(f_)), args(std::forward<As>(as)...) {}
          ...
          protected:
            ...
            constexpr RT bind(Args &&... args) const {
                return std::apply(
                    [&](auto &&... f) {
                        binder<Args...> b { { args... } };
                        return RT { b.template bind<Ts>(std::forward<decltype(f)>(f))... };
                    },
                    self());
            }
            template<typename Char, typename Traits>
            std::basic_ostream<Char, Traits> &print(const Char *label,
    ...
    class any_of: internal::logical_tuple<T...> {
      ...
      public:
        ...
        friend constexpr auto operator==(const U &u, const any_of &a) noexcept(noexcept(a == u))
            -> decltype(a == u) {
            return a == u;
        }
        template<typename U>
    ...
    class none_of: internal::logical_tuple<T...> {
      ...
      public:
        ...
        friend constexpr auto operator==(const U &u, const none_of &a) noexcept(noexcept(a == u))
            -> decltype(a == u) {
            return a == u;
        }
        template<typename U>
    ...
    class all_of: internal::logical_tuple<T...> {
      ...
      public:
        ...
        friend constexpr auto operator==(const U &u, const all_of &a) noexcept(noexcept(a == u))
            -> decltype(a == u) {
            return a == u;
        }
        template<typename U>
