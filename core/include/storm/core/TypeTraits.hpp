...
namespace storm::core {
	...
	template <typename, typename T> struct has_method {
		static_assert(
	...
	template <typename T>
	inline constexpr bool has_const_iterator_v =
		is_detected_v<const_iterator_t, T>;

	template <typename T> using cbegin_t = decltype(std::declval<T>().cbegin());

	template <typename T>
	inline constexpr bool has_cbegin_v = is_detected_v<cbegin_t, T>;

	template <typename T> using cend_t = decltype(std::declval<T>().cend());
	...
	template <typename T>
	struct is_container
		: std::integral_constant<bool, has_const_iterator_v<T> &&
										   has_cbegin_v<T> && has_cend_v<T>> {};
