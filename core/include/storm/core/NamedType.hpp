...
namespace storm::core {
    ...
    class NamedType: public NamedTypeTrait, public Skills<NamedType<T, Tag, Skills...>>... {
      public:
        ...
        /// existing value
        explicit constexpr NamedType(T &&value) noexcept : m_value { std::forward<T>(value) } {}

        /// \output_section Public Member Functions
        /// \brief Get the underlying value as reference
        /// \returns a reference to the underlying value
        constexpr inline T &get() noexcept { return m_value; }
        ...
        /// \returns a reference to the underlying value
        constexpr inline operator T &() noexcept { return get(); }

        /// \brief cast to the underlying value as const reference
        /// \returns a const reference to the underlying value
        constexpr inline operator const T &() const noexcept { return get(); }

        /// \brief cast to the underlying value as copy
        /// \returns a copy of the underlying value
        constexpr inline operator const T() const noexcept { return m_value; }

        constexpr inline const T &operator*() const noexcept { return get(); }

        constexpr inline T &operator*() noexcept { return get(); }
