...
namespace stormkit::gpu {
    ...
    auto CommandBuffer::begin_render_pass(const RenderPass&           render_pass,
                                          ...
                                          bool                        secondary_commandbuffers) noexcept -> CommandBuffer& {
        EXPECTS(m_state == State::RECORDING);

        const auto vk_clear_values = clear_values
    ...
    /////////////////////////////////////
    /////////////////////////////////////
    auto CommandBuffer::set_viewport(u32 first_viewport, std::span<const Viewport> viewports) noexcept -> CommandBuffer& {
        EXPECTS(m_state == State::RECORDING);

        const auto vk_viewports = viewports | stdv::transform(monadic::to_vk()) | stdr::to<std::vector>();
    ...
    /////////////////////////////////////
    /////////////////////////////////////
    auto CommandBuffer::set_scissor(u32 first_scissor, std::span<const Scissor> scissors) noexcept -> CommandBuffer& {
        EXPECTS(m_state == State::RECORDING);

        const auto vk_scissors = scissors | stdv::transform(monadic::to_vk()) | stdr::to<std::vector>();

        vk_call(m_vk_device_table->vkCmdSetScissor, m_vk_handle, first_scissor, stdr::size(vk_scissors), stdr::data(vk_scissors));
        return *this;
    }

    /////////////////////////////////////
    /////////////////////////////////////
    auto CommandBuffer::bind_vertex_buffers(std::span<const Ref<const Buffer>> buffers, std::span<const u64> offsets) noexcept
      -> CommandBuffer& {
        EXPECTS(m_state == State::RECORDING);
        EXPECTS(not std::empty(buffers));
        EXPECTS(std::size(buffers) == std::size(offsets));

        const auto vk_buffers = buffers | stdv::transform(monadic::to_vk()) | stdr::to<std::vector>();
    ...
    auto CommandBuffer::bind_descriptor_sets(const Pipeline&                           pipeline,
        ...
        const auto vk_descriptor_sets = descriptor_sets | stdv::transform(monadic::to_vk()) | stdr::to<std::vector>();
    ...
    /////////////////////////////////////
    /////////////////////////////////////
    auto CommandBuffer::copy_buffer(const Buffer& src, const Buffer& dst, usize size, u64 src_offset, u64 dst_offset) noexcept
      -> CommandBuffer& {
    ...
    auto CommandBuffer::copy_buffer_to_image(const Buffer&                    src,
        ...
        const auto vk_copy_regions = buffer_image_copies
    ...
    auto CommandBuffer::pipeline_barrier(PipelineStageFlag                    src_mask,
        ...
        const auto vk_buffer_memory_barriers = buffer_memory_barriers
                                               | stdv::transform([](auto&& barrier) noexcept -> decltype(auto) {
        ...
        const auto vk_image_memory_barriers = image_memory_barriers
                                              | stdv::transform([](auto&& barrier) noexcept -> decltype(auto) {
                                                    const auto vk_subresource_range = VkImageSubresourceRange {
    ...
    /////////////////////////////////////
    /////////////////////////////////////
    auto CommandBuffer::execute_sub_command_buffers(std::span<const Ref<const CommandBuffer>> commandbuffers) noexcept
      -> CommandBuffer& {
        EXPECTS(m_state == State::RECORDING);
        ...
        const auto vk_command_buffers = commandbuffers
                                        ...
                                        | stdv::transform(core::monadic::map(EXPECTS_secondary, monadic::to_vk()))
                                        | stdr::to<std::vector>();
