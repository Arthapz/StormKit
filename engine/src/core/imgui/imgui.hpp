...
// your own filename)
    ...
    // IMGUI_API is used for core imgui functions, IMGUI_IMPL_API is used for the default bindings
    // files (imgui_impl_xxx.h) Using dear imgui via a shared library is not recommended, because of
    // function call overhead and because we don't guarantee backward nor forward ABI compatibility.
    #ifndef IMGUI_API
        #define IMGUI_API
    #endif
    ...
    // Helper Macros
        ...
        #define IM_FMTARGS(FMT) \
            __attribute__((     \
                format(printf, FMT, FMT + 1))) // To apply printf-style warnings to our functions.
    ...
    #define IM_UNICODE_CODEPOINT_MAX   \
        (sizeof(ImWchar) == 2 ? 0xFFFF \
                              : 0x10FFFF)       // Last Unicode code point supported by this build.
...
struct ImGuiTextFilter; // Helper to parse and apply text filters (e.g. "aaaaa[,bbbb][,ccccc]")
    ...
    // Typedefs and Enums/Flags (declared as int for compatibility with old C++, to allow using as
    // flags and to not pollute the top of this file) Use your programming IDE "Go to definition"
    // facility on the names in the central column below to find the actual flags/enum lists.
    #ifndef ImTextureID
...
typedef unsigned int ImWchar32; // A single U32 character for keyboard input/display. Define ImWchar
                                // to ImWchar32 to use it. See imconfig.h .
...
namespace ImGui {
    ...
    IMGUI_API void ShowMetricsWindow(
        bool *p_open = NULL); // create Debug/Metrics window. display Dear ImGui internals: draw
    ...
    //   value [as with Begin: this is due to legacy reason and inconsistent with most BeginXXX
    //   functions apart from the regular Begin() which behaves like BeginChild().]
    ...
    IMGUI_API bool IsWindowFocused(
        ImGuiFocusedFlags flags = 0); // is current window focused? or its root/child, depending
    ...
    IMGUI_API void SetWindowFontScale(
        float scale); // set font scale. Adjust IO.FontGlobalScale if you want to scale all
    ...
    IMGUI_API const ImVec4 &GetStyleColorVec4(
        ImGuiCol idx); // retrieve style color as stored in ImGuiStyle structure. use to feed
                       // back into PushStyleColor(), otherwise use GetColorU32() to get style
    ...
    IMGUI_API ImU32
        GetColorU32(ImGuiCol idx,
                    float alpha_mul = 1.0f); // retrieve given style color with style alpha applied
                                             // and optional extra alpha multiplier
    ...
    IMGUI_API void PushItemWidth(
        float item_width); // push width of items for common large "item+label" widgets. >0.0f:
                           ...
                           // -1.0f always align width to the right side). 0.0f = default to
                           // ~2/3 of windows width,
    IMGUI_API void PopItemWidth();
    IMGUI_API void SetNextItemWidth(
        float item_width); // set width of the _next_ common large "item+label" widget. >0.0f:
                           // width in pixels, <0.0f align xx pixels to the right of window (so
    ...
    IMGUI_API void PushAllowKeyboardFocus(
        bool allow_keyboard_focus); // allow focusing using TAB/Shift-TAB, enabled by default
    ...
    IMGUI_API bool ImageButton(
        ...
        const ImVec4 &tint_col = ImVec4(1, 1, 1, 1)); // <0 frame_padding uses default frame
    ...
    IMGUI_API bool ColorButton(const char *desc_id,
                               ...
                               ImVec2 size               = ImVec2(0,
                                                    0)); // display a colored square/button, hover
                                                         // for details, return true when pressed.
    IMGUI_API void SetColorEditOptions(
        ImGuiColorEditFlags
            flags); // initialize current options (generally on application startup) if you want
    ...
    // Widgets: List Boxes
    // - FIXME: To be consistent with all the newer API, ListBoxHeader/ListBoxFooter should in
    // reality be called BeginListBox/EndListBox. Will rename them.
    ...
    // - We default to use the right mouse (ImGuiMouseButton_Right=1) for the Popup Context
    // functions.
    ...
    IMGUI_API bool BeginPopupContextItem(
        ...
        ImGuiMouseButton mouse_button =
            1); // helper to open and begin popup when clicked on last item. if you can pass a
                // NULL str_id only if the previous item had an id. If you want to use that on a
                // non-interactive item such as Text() you need to pass in an explicit ID here.
    ...
    IMGUI_API bool BeginPopupContextVoid(
        const char *str_id            = NULL,
        ImGuiMouseButton mouse_button = 1); // helper to open and begin popup when clicked in
    ...
    IMGUI_API void
        SetKeyboardFocusHere(int offset = 0); // focus keyboard on the next widget. Use positive
    ...
    IMGUI_API bool IsMouseDoubleClicked(
        ImGuiMouseButton button); // did mouse button double-clicked? a double-click returns
    ...
    // SetCurrentContext() + SetAllocatorFunctions() again because we use global storage for those.
...
enum ImGuiWindowFlags_ {
    ...
    ImGuiWindowFlags_UnsavedDocument =
        1
        << 20, // Append '*' to title without affecting the ID, as a convenience to avoid using the
...
struct ImGuiIO {
    ...
    bool MouseDrawCursor; // = false          // Request ImGui to draw a mouse cursor for you (if
                          ...
                          // to 'io.ConfigXXX' because this is frequently used by back-end
                          // implementations.
    ...
    bool MouseDown[5]; // Mouse buttons: 0=left, 1=right, 2=middle + extras. ImGui itself mostly
                       // only uses left button (BeginPopupContext** are using right button). Others
    ...
    IMGUI_API void AddInputCharacterUTF16(ImWchar16 c); // Queue new character input from an UTF-16
    ...
    bool WantCaptureKeyboard; // When io.WantCaptureKeyboard is true, imgui will use the keyboard
                              ...
                              // cases, always pass keyboard inputs to imgui). (e.g. InputText
...
struct ImColor {
    ...
    // (in imconfig.h) + update rendering back-end accordingly.
    #ifndef ImDrawCallback
typedef void (*ImDrawCallback)(const ImDrawList *parent_list, const ImDrawCmd *cmd);
    #endif

    // Special Draw callback value to request renderer back-end to reset the graphics/render state.
    // The renderer back-end needs to handle this special value, otherwise it will crash trying to
...
struct ImDrawCmd {
    ...
    // back-end) (to use 32-bit indices: override with '#define ImDrawIdx unsigned int' in
    // imconfig.h)
    #ifndef ImDrawIdx
...
struct ImFontAtlasCustomRect {
    unsigned int ID; // Input    // User ID. Use < 0x110000 to map into a font glyph, >= 0x110000
    ...
    ImVec2 GlyphOffset;  // Input    // For custom font glyphs only (ID < 0x110000): glyph display
                         // offset
...
struct ImFontAtlas {
    ...
    IMGUI_API ImFont *AddFontFromMemoryTTF(
        ...
        const ImWchar *glyph_ranges =
            NULL); // Note: Transfer ownership of 'ttf_data' to ImFontAtlas! Will be deleted
    ...
    // docs/FONTS.txt for more details about using colorful icons.
...
struct ImFont {
    ...
    ImU8 Used4kPagesMap[(IM_UNICODE_CODEPOINT_MAX + 1) / 4096 /
                        8]; // 2 bytes if ImWchar=ImWchar16, 34 bytes if ImWchar==ImWchar32. Store
                            ...
                            // This is mainly used to facilitate iterations accross all used
                            // codepoints.
