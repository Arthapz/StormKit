...
// examples/ are doing that. Read imgui.cpp for more details, documentation and comments. Get latest
// version at https://github.com/ocornut/imgui
...
// - We never use maths operators on ImVec2/ImVec4. For other of our sources files, they are
// provided by imgui_internal.h w/ IMGUI_DEFINE_MATH_OPERATORS.
    ...
    #include <stdlib.h>                       // NULL, malloc, free, atoi
        ...
        #pragma clang diagnostic ignored \
            "-Wexit-time-destructors" // warning : declaration requires an exit-time destructor //
                                      ...
                                      // static/globals.
        #pragma clang diagnostic ignored \
            "-Wunused-macros" // warning : warning: macro is not used                         // we
        ...
        #pragma GCC diagnostic ignored \
            "-Wmisleading-indentation" // [__GNUC__ >= 6] warning: this 'if' clause does not
                                       // guard this statement      // GCC 6.0+ only. See
                                       // #883 on GitHub.
    #endif
...
void ImGui::ShowDemoWindow(bool *p_open) {
    IM_ASSERT(
        ...
        "Missing dear imgui context. Refer to examples app!"); // Exceptionally add an extra
                                                               // assert here for people confused
                                                               // with initial dear imgui setup
    ...
    if (ImGui::CollapsingHeader("Configuration")) {
        ...
        if (ImGui::TreeNode("Configuration##2")) {
            ...
            ImGui::SameLine();
            HelpMarker("Required back-end to feed in gamepad inputs in io.NavInputs[] and set "
...
static void ShowDemoWindowWidgets() {
    ...
    if (ImGui::TreeNode("Text")) {
        ...
        if (ImGui::TreeNode("UTF-8 Text")) {
            ...
            // (Needs a suitable font, try Noto, or Arial Unicode, or M+ fonts. Read docs/FONTS.txt
            // for details.)
            ...
            ImGui::TextWrapped(
                ...
                "character ranges. Read docs/FONTS.txt for details.");
    ...
    if (ImGui::TreeNode("Images")) {
        ...
        // 'ID3D11ShaderResourceView*' pointer. The imgui_impl_opengl3.cpp renderer expect a GLuint
            ...
            for (int n = 0; n < IM_ARRAYSIZE(items); n++) {
                ...
                if (is_selected)
                    ImGui::SetItemDefaultFocus(); // Set the initial focus when opening the combo
                                                  // (scrolling + for keyboard navigation support in
                                                  // the upcoming navigation branch)
            }
            ImGui::EndCombo();
        }
    ...
    if (ImGui::TreeNode("Text Input")) {
        ...
        if (ImGui::TreeNode("Resize Callback")) {
            ...
            // ImGui::InputText() wrapper using your prefered type. See misc/cpp/imgui_stdlib.h for
    ...
    // Consider writing your own plotting widget, or using a third-party one (see "Wiki->Useful
    // Widgets", or github.com/ocornut/imgui/issues/2747)
    ...
    if (ImGui::TreeNode("Data Types")) {
        ...
        // signed/unsigned int/long long and float/double To avoid polluting the public API with all
        // possible combinations, we use the ImGuiDataType enum to pass the type, and passing all
        // arguments by address. This is the reason the test code below creates local variables to
        // hold "zero" "one" etc. for each types. In practice, if you frequently use a given type
        // that is not covered by the normal API entry points, you can wrap it yourself inside a 1
        // line function which can take typed argument as value instead of void*, and then pass
        // their address to the generic function. For example:
        ...
        // Note that the SliderScalar function has a maximum usable range of half the natural type
        // maximum, hence the /2 below.
        #ifndef LLONG_MIN
    ...
    if (ImGui::TreeNode("Drag and Drop")) {
        ...
        if (ImGui::TreeNode("Drag to reorder items (simple)")) {
            // Simple reordering
            HelpMarker(
                "We don't use the drag and drop api at all here! Instead we query when the item is "
                "held but not hovered, and order items accordingly.");
    ...
    if (ImGui::TreeNode("Querying Status (Active/Focused/Hovered etc.)")) {
        // Submit an item (various types available) so we can query their status in the following
        // block.
        ...
        ImGui::Combo("Item Type",
                     ...
                     "repeat)"
                     "\0Checkbox\0SliderFloat\0InputText\0InputFloat\0InputFloat3\0ColorEdit4\0Menu"
                     "Item\0TreeNode\0TreeNode (w/ double-click)\0ListBox\0",
                     20);
        ImGui::SameLine();
        HelpMarker(
...
static void ShowDemoWindowLayout() {
    ...
    if (ImGui::TreeNode("Text Baseline Alignment")) {
        {
            ...
            HelpMarker(
                ...
                "aligned with widgets. Lines only composed of text or \"small\" widgets fit in "
            ...
            // need to move the text baseline down by FramePadding.y
            ImGui::AlignTextToFramePadding();
    ...
    if (ImGui::TreeNode("Scrolling")) {
        ...
        ImGui::SameLine();
        HelpMarker("We expose this for testing because scrolling sometimes had issues with window "
                   "decoration such as menu-bars.");
        ...
        for (int i = 0; i < 5; i++) {
            ...
            bool window_visible          = ImGui::BeginChild(ImGui::GetID((void *)(intptr_t)i),
...
void ImGui::ShowAboutWindow(bool *p_open) {
    ...
    if (show_config_info) {
        ...
        if (copy_to_clipboard) {
            ImGui::LogToClipboard();
            ImGui::LogText("```\n"); // Back quotes will make the text appears without formatting
                                     // when pasting to GitHub
        }
...
void ImGui::ShowStyleEditor(ImGuiStyle *ref) {
    ...
    if (ImGui::BeginTabBar("##tabs", ImGuiTabBarFlags_None)) {
        ...
        if (ImGui::BeginTabItem("Colors")) {
            ...
            for (int i = 0; i < ImGuiCol_COUNT; i++) {
                ...
                if (memcmp(&style.Colors[i], &ref->Colors[i], sizeof(ImVec4)) != 0) {
                    ...
                    // FAQ and docs/FONTS.txt about using icon fonts. It's really easy and super
                    // convenient!
        ...
        if (ImGui::BeginTabItem("Fonts")) {
                ...
                if (font_details_opened) {
                    ...
                    if (ImGui::TreeNode("Glyphs", "Glyphs (%d)", font->Glyphs.Size)) {
                        ...
                        for (unsigned int base = 0; base <= IM_UNICODE_CODEPOINT_MAX; base += 256) {
                            ...
                            // the number of iterations when IM_UNICODE_MAX_CODEPOINT is large. (if
            ...
            HelpMarker(
                "Those are old settings provided for convenience.\nHowever, the _correct_ way of "
                "scaling your UI is currently to reload your font at the designed size, rebuild "
                "the font atlas, and call style.ScaleAllSizes() on a reference ImGuiStyle "
                "structure.");
            ...
            if (node_open) {
                ...
                for (int i = 0; i < 8; i++) {
                    ...
                    if (i < 2) {
                        ...
                        ImGui::NextColumn();
                        ImGui::SetNextItemWidth(-1);
                        if (i >= 5) ImGui::InputFloat("##value", &dummy_members[i], 1.0f);
                        else
    ...
    if (ImGui::BeginTabBar("##TabBar")) {
        if (ImGui::BeginTabItem("Primitives")) {
            ...
            ImVec2 gradient_size = ImVec2(ImGui::CalcItemWidth(), ImGui::GetFrameHeight());
            {
                ...
                draw_list->AddRectFilledMultiColor(p,
                                                   ImVec2(p.x + gradient_size.x,
                                                          p.y + gradient_size.y),
                                                   col_a,
                                                   col_b,
                                                   col_b,
            ...
            for (int n = 0; n < 2; n++) {
                ...
                x += sz + spacing; // Circle
                draw_list
                    ->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz), col, 0.0f, corners_none, th);
                x += sz + spacing; // Square
                draw_list
                    ->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz), col, 10.0f, corners_all, th);
                x += sz + spacing; // Square with all rounded corners
                draw_list
                    ->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz), col, 10.0f, corners_tl_br, th);
