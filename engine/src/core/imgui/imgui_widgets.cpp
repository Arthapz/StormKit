...
bool ImGui::ImageButton(ImTextureID user_texture_id,
    ...
    window->DrawList
        ->AddImage(user_texture_id, image_bb.Min, image_bb.Max, uv0, uv1, GetColorU32(tint_col));
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type,
    ...
    if (is_power) {
        ...
        v_cur                       = v_min +
                (SIGNEDTYPE)ImPow(ImSaturate((float)v_new_norm_curved), power) * (v_max - v_min);
...
bool ImGui::SliderBehaviorT(const ImRect &bb,
        ...
        if (set_new_value) {
            ...
            if (is_power) {
                ...
                if (is_decimal) {
                    ...
                    if (v_new_off_floor < v_new_off_round) v_new = v_min + v_new_off_round;
                    else
    ...
    if (slider_sz < 1.0f) {
        *out_grab_bb = ImRect(bb.Min, bb.Min);
        ...
        float grab_t = SliderCalcRatioFromValueT<TYPE, FLOATTYPE>(data_type,
                                                                  *v,
                                                                  v_min,
                                                                  v_max,
                                                                  power,
...
bool ImGui::InputFloat4(const char *label,
    ...
    // Prefer using "const char* format" directly, which is more flexible and consistent with other
    // API.
    #ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
bool ImGui::InputFloat(const char *label,
                       float *v,
...
bool ImGui::BeginMenu(const char *label, bool enabled) {
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        float w        = window->DC.MenuColumns.DeclColumns(label_size.x,
                                                     0.0f,
                                                     IM_FLOOR(g.FontSize *
