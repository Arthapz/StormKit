...
bool EntityManager::hasEntity(Entity entity) {
    ...
    auto it = std::find(std::cbegin(m_entities), std::cend(m_entities), entity);
    auto it2 = std::find(std::cbegin(m_added_entities),
                         std::cend(m_added_entities), entity);
...
void EntityManager::step(uint64_t delta) {
    for (auto entity : m_added_entities) {
    ...
    for (auto entity : m_removed_entities) {
        auto it =
            std::find(std::begin(m_entities), std::end(m_entities), entity);
    ...
    while (!m_message_bus->empty()) {
        for (auto &system : m_systems)
            system->onMessageReceived(m_message_bus->top());
        m_message_bus->pop();
    }
