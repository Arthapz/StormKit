...
namespace storm::image {
	Image::Codec filenameToCodec(const core::filesystem::path &filename) {
...
/////////////////////////////////////
/////////////////////////////////////
Image::Image(const core::filesystem::path &filepath, Image::Codec codec)
	: Image{} {
...
/////////////////////////////////////
/////////////////////////////////////
void Image::loadFromFile(const core::filesystem::path &filepath,
						 Image::Codec codec) {
	detach();
...
void Image::loadFromMemory(Image::const_span data, Image::Codec codec) {
	...
	switch (codec) {
	case Image::Codec::JPEG:
		m_data = private_::ImageLoader::loadJpeg(data);
		break;
	case Image::Codec::PNG:
		m_data = private_::ImageLoader::loadPng(data);
		break;
	case Image::Codec::TARGA:
		m_data = private_::ImageLoader::loadTga(data);
		break;
	case Image::Codec::PPM:
		m_data = private_::ImageLoader::loadPPM(data);
		break;
	default:
		throw std::runtime_error("oops");
	}
...
/////////////////////////////////////
/////////////////////////////////////
void Image::create(std::uint32_t width, std::uint32_t height,
...
Image::span Image::operator()(XOffset x_offset, YOffset y_offset) {
	STORM_EXPECTS(m_data != nullptr);
	STORM_EXPECTS(x_offset < m_data->extent.width &&
				  y_offset < m_data->extent.height);

	detach();

	auto pointer =
		&m_data->data[(y_offset * extent().width + x_offset) * channels()];
...
/////////////////////////////////////
/////////////////////////////////////
Image::const_span Image::operator()(XOffset x_offset, YOffset y_offset) const
	noexcept {
	STORM_EXPECTS(m_data != nullptr);
	STORM_EXPECTS(x_offset < m_data->extent.width &&
				  y_offset < m_data->extent.height);
...
Image Image::flipX(const Image &src) {
	...
	std::fill(std::begin(out_data), std::end(out_data), std::byte{0});

	for (auto x = 0u; x < out.extent().width; ++x)
		for (auto y = 0u; y < out.extent().height; ++y)
			for (auto k = 0u; k < src.channels(); ++k) {
				const auto out_index =
					gsl::narrow_cast<Image::const_span::index_type>(
						(x + y * out.extent().width) * src.channels() + k);
				const auto src_index =
					gsl::narrow_cast<Image::const_span::index_type>(
						(x + (out.extent().height) * out.extent().width - 1u -
						 x) *
							src.channels() +
						k);
		...
		for (auto y = 0u; y < out.extent().height; ++y)
			for (auto k = 0u; k < src.channels(); ++k) {
				...
				const auto src_index =
					gsl::narrow_cast<Image::const_span::index_type>(
						(x +
						 (out.extent().height - 1u - y) * out.extent().width) *
							src.channels() +
						k);
...
/////////////////////////////////////
/////////////////////////////////////
void Image::saveToFile(const core::filesystem::path &filename, Codec codec,
					   CodecArgs args) {
	STORM_EXPECTS(m_data != nullptr);
...
void Image::addChannels(std::uint8_t count) {
	...
	for (auto i = 0u; i < pixel_count; ++i) {
		...
		for (auto j = 0u; j < count; ++j) image_data.emplace_back(std::byte{});
	}
