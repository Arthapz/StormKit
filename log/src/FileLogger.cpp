...
////////////////////////////////////////
////////////////////////////////////////
FileLogger::FileLogger(LogClock::time_point start, core::filesystem::path path,
					   Severity log_level)
	: Logger{std::move(start), log_level}, m_base_path{std::move(path)} {
	if (!core::filesystem::exists(m_base_path))
		core::filesystem::create_directory(m_base_path);

	ASSERT(core::filesystem::is_directory(m_base_path),
...
void FileLogger::write(Severity severity, Module module, const char *string) {
	...
	if (std::strcmp(module.get(), "") != 0) {
		...
		if (m_streams.find(filepath.string()) == std::end(m_streams))
			m_streams[filepath.string()] = std::ofstream{filepath.string()};
	}
