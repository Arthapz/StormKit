...
void WindowImpl::create(const std::string &title, const VideoSettings &settings,
	...
	std::uint32_t value_list[] = {screen->black_pixel, screen->black_pixel,
								  EVENTS};
	...
	if ((style & WindowStyle::Fullscreen) == WindowStyle::Fullscreen) {
		auto wm_state =
			xcb_intern_atom(m_connection.get(), 0, 13, "_NET_WM_STATE");
		auto wm_state_reply =
			xcb_intern_atom_reply(m_connection.get(), wm_state, nullptr);
		auto wm_state_fs = xcb_intern_atom(m_connection.get(), 0, 24,
										   "_NET_WM_STATE_FULLSCREEN");
		auto wm_state_fs_reply =
			xcb_intern_atom_reply(m_connection.get(), wm_state_fs, nullptr);

		xcb_change_property(m_connection.get(), XCB_PROP_MODE_REPLACE, m_window,
							wm_state_reply->atom, XCB_ATOM_ATOM, 32, 1,
		...
		if (atom_reply &&
			...
			xcb_change_property(m_connection.get(), XCB_PROP_MODE_REPLACE,
								m_window, atom_reply->atom, XCB_ATOM_WM_HINTS,
								32, 5, reinterpret_cast<std::byte *>(&hints));
		...
		if ((style & WindowStyle::Resizable) != WindowStyle::Resizable) {
			...
			xcb_icccm_set_wm_normal_hints(m_connection.get(), m_window,
										  &size_hints);
		}
	}
	...
	m_protocol_cookie =
		xcb_intern_atom(m_connection.get(), 1, 12, "WM_PROTOCOLS");
	m_protocol_reply =
		xcb_intern_atom_reply(m_connection.get(), m_protocol_cookie, nullptr);

	m_close_cookie =
		xcb_intern_atom(m_connection.get(), 0, 16, "WM_DELETE_WINDOW");
	m_close_reply =
		xcb_intern_atom_reply(m_connection.get(), m_close_cookie, nullptr);
	m_video_settings = settings;

	xcb_change_property(m_connection.get(), XCB_PROP_MODE_REPLACE, m_window,
						m_protocol_reply->atom, 4, 32, 1, &m_close_reply->atom);
...
storm::core::Extent WindowImpl::size() const noexcept {
	const auto reply = xcb_get_geometry_reply(
		m_connection.get(), xcb_get_geometry(m_connection.get(), m_window),
		nullptr);

    auto result = core::Extent{reply->width, reply->height};
	free(reply);
...
void WindowImpl::processEvents(xcb_generic_event_t xevent, void *native_event) {
	...
	case XCB_MOTION_NOTIFY: {
		...
		m_mouse_y = mouse_event->event_y;
		AbstractWindow::mouseMoveEvent(mouse_event->event_x,
									   mouse_event->event_y);
		break;
	}
	...
	case XCB_CONFIGURE_NOTIFY: {
		auto configure_event =
			reinterpret_cast<xcb_configure_notify_event_t *>(&xevent);

		if ((configure_event->width != m_video_settings.size.w) ||
			(configure_event->height != m_video_settings.size.h))
			AbstractWindow::resizeEvent(configure_event->width,
	...
	case XCB_CLIENT_MESSAGE:
		if (reinterpret_cast<xcb_client_message_event_t *>(&xevent)
				->data.data32[0] == m_close_reply->atom)
