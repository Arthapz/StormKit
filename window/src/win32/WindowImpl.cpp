...
std::wstring fromStdString(std::string_view str) {
	...
	MultiByteToWideChar(CP_UTF8, 0, std::data(str), -1, std::data(wstr),
...
void WindowImpl::create(const std::string &title, const VideoSettings &settings,
	...
	m_window_handle =
		CreateWindowExW(0l, CLASS_NAME, fromStdString(title).c_str(), _style,
						CW_USEDEFAULT, CW_USEDEFAULT, settings.size.w,
						settings.size.h, nullptr, nullptr, h_instance, this);
...
void WindowImpl::setVideoSettings(
	...
	auto rectangle = RECT{0, 0, static_cast<long>(settings.size.w),
						  static_cast<long>(settings.size.h)};

	AdjustWindowRect(&rectangle, GetWindowLongW(m_window_handle, GWL_STYLE),
					 false);
	auto width  = rectangle.right - rectangle.left;
	auto height = rectangle.bottom - rectangle.top;

	SetWindowPos(m_window_handle, nullptr, 0, 0, width, height,
				 SWP_NOMOVE | SWP_NOZORDER);
}

/////////////////////////////////////
/////////////////////////////////////
core::Extent WindowImpl::size() const noexcept {
	RECT rect;
	GetClientRect(m_window_handle, &rect);

	return {gsl::narrow_cast<std::uint32_t>(rect.right - rect.left),
			 gsl::narrow_cast<std::uint32_t>(rect.bottom - rect.top)};
}
...
/////////////////////////////////////
/////////////////////////////////////
LRESULT WindowImpl::globalOnEvent(HWND handle, UINT message, WPARAM w_param,
								  LPARAM l_param) {
	if (message == WM_CREATE) {
		auto lp_create_params =
			reinterpret_cast<CREATESTRUCT *>(l_param)->lpCreateParams;

		SetWindowLongPtrW(handle, GWLP_USERDATA,
						  reinterpret_cast<LONG_PTR>(lp_create_params));
	}

	auto window = handle ? reinterpret_cast<WindowImpl *>(
							   GetWindowLongPtrW(handle, GWLP_USERDATA))
						 : nullptr;
	if (window) {
		window->processEvents(message, w_param, l_param);
	}

	if (message == WM_CLOSE)
		return 0;

	if ((message == WM_SYSCOMMAND) && (w_param == SC_KEYMENU))
		return 0;
...
void WindowImpl::processEvents(UINT message, WPARAM w_param, LPARAM l_param) {
	...
	case WM_EXITSIZEMOVE:
		...
		break;
	case WM_LBUTTONDOWN: {
		auto x = static_cast<std::int16_t>(LOWORD(l_param));
		auto y = static_cast<std::int16_t>(HIWORD(l_param));
	...
	case WM_MOUSEMOVE: {
		...
		if ((w_param & (MK_LBUTTON | MK_MBUTTON | MK_RBUTTON | MK_XBUTTON1 |
						MK_XBUTTON2)) == 0) {
			if (GetCapture() == m_window_handle)
				ReleaseCapture();
		} else if (GetCapture() != m_window_handle)
			SetCapture(m_window_handle);

		if ((x < area.left) || (x > area.right) || (y < area.top) ||
			(y > area.bottom)) {
			if (m_mouse_inside) {
				m_mouse_inside = false;
