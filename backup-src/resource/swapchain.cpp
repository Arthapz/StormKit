...
namespace stormkit::gpu {
    ...
    Swapchain::Swapchain(Tag,
        ...
        const auto  capabilities    = physical_device.vkHandle()
                                    .getSurfaceCapabilitiesKHR(*surface.vkHandle());
        const auto formats = physical_device.vkHandle().getSurfaceFormatsKHR(*(surface.vkHandle()));
        const auto present_modes = physical_device.vkHandle()
                                     .getSurfacePresentModesKHR(*(surface.vkHandle()));
    ...
    auto Swapchain::acquireNextImage(std::chrono::nanoseconds wait,
      ...
      -> Expected<std::pair<gpu::Result, UInt32>> {
        auto&& [result, index] = m_vk_swapchain
                                   ->acquireNextImage(wait.count(), to_vkhandle(image_available));
