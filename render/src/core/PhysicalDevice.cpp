...
std::string vendorNameByID(std::uint64_t ID) {
	switch (ID) {
	case 0x1002:
		return "AMD";
	case 0x1010:
...
PhysicalDevice::PhysicalDevice(VkPhysicalDevice vk_physical_device,
	...
	vkGetPhysicalDeviceFeatures(m_vk_physical_device, &features);
	...
	m_device_info.driver_major_version =
		vkVersionMajor(properties.driverVersion);
	m_device_info.driver_minor_version =
		vkVersionMinor(properties.driverVersion);
	m_device_info.driver_patch_version =
		vkVersionPatch(properties.driverVersion);

	m_device_info.type = fromVK(properties.deviceType);

	m_capabilities.limits.max_image_dimension_1D =
		properties.limits.maxImageDimension1D;
	m_capabilities.limits.max_image_dimension_2D =
		properties.limits.maxImageDimension2D;
	m_capabilities.limits.max_image_dimension_3D =
		properties.limits.maxImageDimension3D;
	m_capabilities.limits.max_image_dimension_cube =
		properties.limits.maxImageDimensionCube;
	m_capabilities.limits.max_image_array_layers =
		properties.limits.maxImageArrayLayers;
	m_capabilities.limits.max_texel_buffer_elements =
		properties.limits.maxTexelBufferElements;
	m_capabilities.limits.max_uniform_buffer_range =
		properties.limits.maxUniformBufferRange;
	m_capabilities.limits.max_storage_buffer_range =
		properties.limits.maxStorageBufferRange;
	m_capabilities.limits.max_push_constants_size =
	...
	m_capabilities.limits.buffer_image_granularity =
		properties.limits.bufferImageGranularity;
	m_capabilities.limits.sparse_address_space_size =
		properties.limits.sparseAddressSpaceSize;
	m_capabilities.limits.max_bound_descriptor_sets =
	...
	m_capabilities.limits.max_per_stage_resources =
	...
	m_capabilities.limits.max_vertex_input_bindings =
	...
	std::copy(std::cbegin(properties.limits.maxComputeWorkGroupCount),
			  ...
			  std::begin(m_capabilities.limits.max_compute_work_group_count));
	...
	std::copy(std::cbegin(properties.limits.maxComputeWorkGroupSize),
			  ...
			  std::begin(m_capabilities.limits.max_compute_work_group_size));
	m_capabilities.limits.sub_pixel_precision_bits =
		properties.limits.subPixelPrecisionBits;
	m_capabilities.limits.sub_texel_precision_bits =
		properties.limits.subTexelPrecisionBits;
	m_capabilities.limits.mipmap_precision_bits =
	...
	m_capabilities.limits.max_draw_indirect_count =
		properties.limits.maxDrawIndirectCount;
	m_capabilities.limits.max_sampler_lod_bias =
		properties.limits.maxSamplerLodBias;
	m_capabilities.limits.max_sampler_anisotropy =
	...
	std::copy(std::cbegin(properties.limits.maxViewportDimensions),
			  ...
			  std::begin(m_capabilities.limits.max_viewport_dimensions));
	std::copy(std::cbegin(properties.limits.viewportBoundsRange),
			  std::cend(properties.limits.viewportBoundsRange),
			  std::begin(m_capabilities.limits.viewport_bounds_range));
	m_capabilities.limits.viewport_sub_pixel_bits =
		properties.limits.viewportSubPixelBits;
	m_capabilities.limits.min_memory_map_alignment =
	...
	m_capabilities.limits.min_texel_gather_offset =
		properties.limits.minTexelGatherOffset;
	m_capabilities.limits.max_texel_gather_offset =
		properties.limits.maxTexelGatherOffset;
	m_capabilities.limits.min_interpolation_offset =
		properties.limits.minInterpolationOffset;
	m_capabilities.limits.max_interpolation_offset =
	...
	m_capabilities.limits.max_framebuffer_width =
		properties.limits.maxFramebufferWidth;
	m_capabilities.limits.max_framebuffer_height =
		properties.limits.maxFramebufferHeight;
	m_capabilities.limits.max_framebuffer_layers =
		properties.limits.maxFramebufferLayers;
	m_capabilities.limits.framebuffer_color_sample_counts =
		fromVK(core::makeNamed<VkSampleCountFlagsType>(
	...
	m_capabilities.limits.max_color_attachments =
	...
	m_capabilities.limits.discrete_queue_priorities =
	...
	m_capabilities.features.draw_indirect_first_instance =
	...
	m_capabilities.features.texture_compression_etc2 =
		features.textureCompressionETC2;
	m_capabilities.features.texture_compression_astc_ldr =
		features.textureCompressionASTC_LDR;
	m_capabilities.features.texture_compression_bc =
		features.textureCompressionBC;
	m_capabilities.features.occlusion_query_precise =
		features.occlusionQueryPrecise;
	m_capabilities.features.pipeline_statistics_query =
		features.pipelineStatisticsQuery;
	m_capabilities.features.vertex_pipeline_stores_and_atomics =
		features.vertexPipelineStoresAndAtomics;
	m_capabilities.features.fragment_stores_and_atomics =
		features.fragmentStoresAndAtomics;
	m_capabilities.features.shader_tessellation_and_geometry_point_size =
		features.shaderTessellationAndGeometryPointSize;
	m_capabilities.features.shader_image_gather_extended =
	...
	m_capabilities.features.shader_resource_residency =
		features.shaderResourceResidency;
	m_capabilities.features.shader_resource_min_lod =
		features.shaderResourceMinLod;
	m_capabilities.features.sparse_binding = features.sparseBinding;
	m_capabilities.features.sparse_residency_buffer =
		features.sparseResidencyBuffer;
	m_capabilities.features.sparse_residency_image_2D =
		features.sparseResidencyImage2D;
	m_capabilities.features.sparse_residency_image_3D =
		features.sparseResidencyImage3D;
	m_capabilities.features.sparse_residency_2_samples =
		features.sparseResidency2Samples;
	m_capabilities.features.sparse_residency_4_samples =
		features.sparseResidency4Samples;
	m_capabilities.features.sparse_residency_8_samples =
		features.sparseResidency8Samples;
	m_capabilities.features.sparse_residency_16_samples =
		features.sparseResidency16Samples;
	m_capabilities.features.sparse_residency_aliased =
		features.sparseResidencyAliased;
	m_capabilities.features.variable_multisample_rate =
		features.variableMultisampleRate;
	m_capabilities.features.inherited_queries = features.inheritedQueries;

	m_device_info.type = fromVK(properties.deviceType);

	auto vk_extension_count = std::uint32_t{0};
	vkEnumerateDeviceExtensionProperties(m_vk_physical_device, nullptr,
										 &vk_extension_count, nullptr);

	auto extensions = std::vector<VkExtensionProperties>{vk_extension_count};
	vkEnumerateDeviceExtensionProperties(m_vk_physical_device, nullptr,
										 &vk_extension_count,
										 std::data(extensions));
	...
	std::transform(std::cbegin(extensions), std::cend(extensions),
				   std::back_inserter(m_extensions), [](const auto &extension) {
					   return extension.extensionName;
				   });

	vkGetPhysicalDeviceMemoryProperties(m_vk_physical_device,
	...
	vkGetPhysicalDeviceQueueFamilyProperties(
		m_vk_physical_device, &queue_family_count,
		std::data(queue_family_properties));

	for (const auto &queue_family : queue_family_properties) {
		if (queue_family.queueCount > 0) {
			auto family = QueueFamily{.flags = fromVK(queue_family.queueFlags),
...
/////////////////////////////////////
/////////////////////////////////////
render::DeviceOwnedPtr PhysicalDevice::createLogicalDevice() const {
	return std::make_unique<Device>(*this, *m_instance);
}

/////////////////////////////////////
/////////////////////////////////////
void PhysicalDevice::checkIfPresentSupportIsEnabled(
	const render::Surface &surface) noexcept {
	const auto &surface_ = static_cast<const Surface &>(surface);

	auto index = 0u;
	for (auto &queue_family : m_queue_families) {
		auto present_support = VkBool32{VK_FALSE};
		vkGetPhysicalDeviceSurfaceSupportKHR(m_vk_physical_device, index++,
...
VkSurfaceCapabilitiesKHR
	PhysicalDevice::queryVkSurfaceCapabilities(const Surface &surface) const
	noexcept {
	auto capabilities = VkSurfaceCapabilitiesKHR{};
	vkGetPhysicalDeviceSurfaceCapabilitiesKHR(m_vk_physical_device, surface,
											  &capabilities);
...
std::vector<VkSurfaceFormatKHR>
	PhysicalDevice::queryVkSurfaceFormats(const Surface &surface) const
	noexcept {
	auto format_count = 0u;
	vkGetPhysicalDeviceSurfaceFormatsKHR(m_vk_physical_device, surface,
										 &format_count, nullptr);

	auto formats = std::vector<VkSurfaceFormatKHR>{format_count};
	vkGetPhysicalDeviceSurfaceFormatsKHR(m_vk_physical_device, surface,
										 &format_count, std::data(formats));
...
std::vector<VkPresentModeKHR>
	...
	vkGetPhysicalDeviceSurfacePresentModesKHR(m_vk_physical_device, surface,
											  &present_mode_count, nullptr);

	auto present_mode = std::vector<VkPresentModeKHR>{present_mode_count};
	vkGetPhysicalDeviceSurfacePresentModesKHR(m_vk_physical_device, surface,
											  &present_mode_count,
											  std::data(present_mode));
...
/////////////////////////////////////
/////////////////////////////////////
bool PhysicalDevice::checkExtensionSupport(gsl::czstring<> extension) const
	noexcept {
	return std::find(std::cbegin(m_extensions), std::cend(m_extensions),
					 extension) != std::cend(m_extensions);
}
